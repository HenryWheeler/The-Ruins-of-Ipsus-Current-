    ...
    public class Stoning : OnTurn
    {
        ...
        public override void Turn()
        {
            ...
            switch (timeLeft)
            {
                case 6:
                    {
                        if (entity.GetComponent<PronounSet>().present) { Log.Add(entity.GetComponent<Description>().name + "'s bones become stiffer"); }
                        else { Log.Add(entity.GetComponent<PronounSet>().possesive + " bones become stiffer"); }
                        break;
                    }
                case 3:
                    {
                        if (entity.GetComponent<PronounSet>().present) { Log.Add(entity.GetComponent<Description>().name + "'s skin turns Gray*gray"); }
                        else { Log.Add(entity.GetComponent<PronounSet>().possesive + " skin turns Gray*gray"); }
                        break;
                    }
                case 0:
                    {
                        if (entity.GetComponent<PronounSet>().present) { Log.Add(entity.GetComponent<Description>().name + "'s body has becomes completely stone"); }
                        else { Log.Add(entity.GetComponent<PronounSet>().possesive + " body has become completely stone"); }
                        ...
                        statue.AddComponent(new ID("Obstacle"));
